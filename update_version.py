...
def update_version_txt(version_txt, current_version, next_version):
    with open(version_txt, mode = "r", encoding = "utf-8") as f:
        src = f.read()
    txt = src.replace(f"Version = `v{current_version}`", f"Version = `v{next_version}`")
    with open(version_txt, mode = "w", encoding = "utf-8") as f:
        f.write(txt)
    print(f"updated {version_txt} Version from v{current_version} to v{next_version}")
    return True 
...
def main(args):
    ...
    if ("--yes" in args) or ("-yes" in args) or ("-y" in args):
        ...
        update_version_txt(VERSION_TXT, current_version, next_version)
    else:
        print("current version:", current_version)
